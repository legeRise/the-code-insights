...
description: "The story of building Ezlog — a simple web-based log viewer after getting tired of SSHing into servers just to check logs."
pubDate: "2026-03-19"
---
...
1. Deploying a webapp on Server
2. Connecting domain
3. Planning backup Mechanism for applications
...
Everything was great until logging into server again and again started to feel not right. This is the moment where I started to wonder If there was a way to view logs directly on the browser and there my search begins. I searched here and there and to be honest, I was either **too naive to get comfortable with any of the famous Log viewers** or **_too developer to act as just a normal user for once._**

Whatever the reason I couldn't find anything until I hopped on to `frontail`

[**GitHub - mthenw/frontail: 📝 streaming logs to the browser. Sponsored by https://cloudash.dev**  
📝 streaming logs to the browser. Sponsored by https://cloudash.dev - mthenw/frontail](https://github.com/mthenw/frontail)
...
![Frontail demo](https://cdn-images-1.medium.com/max/800/0\*YLi_kYDtqnHBj79W.gif)
...
4. Those who were not `Solo` not a `Team` but were a bunch of `individuals` brought together under the umbrella of a yet another `startup` were the ones to struggle the most because few of them used to be
   1. Intellectual, Curious, Interested and wanting everything to be handled the best and recommened way
   2. While others would be too busy to just finish there job
...
[**GitHub - legeRise/ezlog: A simple web-based log viewer**  
A simple web-based log viewer. Contribute to legeRise/ezlog development by creating an account on GitHub.](https://github.com/legeRise/ezlog.git)

Presenting `Ezlog` A simple web based log viewer. Lol hope you already saw that coming.
...
![Ezlog tabs](https://cdn-images-1.medium.com/max/800/0\*X18U8EmRcoOK_-Ol.png)

You can see one tab per log file

2. `Aliases` so each tab has a friendly name that you can use for tracking your project

> for example for your project **my-projects/special/ai-chatbot/chatbot.log**

> you could simply give it a name like **chatbot**

3. **Easy CLI**

One of my favourite features because as I started it, I quickly realized that the hardest part was to add new log file to be tracked
...
![ezlog list command](https://cdn-images-1.medium.com/max/800/1\*F7g6qUovisnHYo9E4FHYyg.png)

This command lists `aliases` -> `paths`

2. Adding a new file to be tracked simple `ezlog add <alias> <file-path>`

![ezlog add command](https://cdn-images-1.medium.com/max/800/1\*0HUV-weL4GeLF7GVMTGjYA.png)
...
![ezlog start command](https://cdn-images-1.medium.com/max/800/1\*m02-KfPpAVysTZGW_-ClMg.png)
...
![Ezlog stop](https://cdn-images-1.medium.com/max/800/0\*enPD87VYR1PMe2KJ.png)
...
[**ezlog/README.md at main · legeRise/ezlog**  
A simple web-based log viewer. Contribute to legeRise/ezlog development by creating an account on GitHub.](https://github.com/legeRise/ezlog/blob/main/README.md)

![Ezlog quickstart](https://cdn-images-1.medium.com/max/800/1\*V2oUM3iUvDU2LzGsyjga9g.png)

Please do try and let me know if you like it.

What's left? There are a few things I already know i am missing and are important like
